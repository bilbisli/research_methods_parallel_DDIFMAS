...
def DMRSD_I1D1R1(instance_num, noa, nof, afp, nor, inum, F, S):
    ...
    ranked_diagnoses.sort(key=lambda diag: diag[1], reverse=True)
    t3 = time.time()
    # ranking time is not part of the experiment comparing to the parallel algorithm
    delta_rank = 0
    # delta_rank = t3 - t2
...
def parallel_DMRSD_I1D1R1(instance_num, noa, nof, afp, nor, inum, F, S):
    """
    :param instance_num: instance number for indexing of experiments
    :param noa: number of agents
    :param nof: number of faulty agents
    ...
    """
    # announcing instance parameters
    print(f'running parallel_DMRSD_I1D1R1 on {instance_num} ({inum}) with:')
    print(f'        - number of agents: {noa}')
    print(f'        - number of faulty agents: {nof}')
    print(f'        - agent fault probability: {afp}')
    print(f'        - number of runs: {nor}')
    ...
    # run the algorithm, collect diagnosis messages count
    print(f'parallel_DMRSD_I1D1R1:: running diagnoses')
    # t0 = time.time()
    diagnoses, info_sent_diagnosis, revealed_information_sum, revealed_information_mean,\
        revealed_information_per_agent, revealed_information_last, revealed_information_percent_per_agent, \
        revealed_information_percent_last, current_cumulative_max_time, number_of_steps \
        = methods_for_diagnosis.semi_parallel_diagnosis_1(local_spectra, missing_information_cells)
    # t1 = time.time()
    delta_diag = current_cumulative_max_time
    # print(f'parallel_DMRSD_I1D1R1:: diagnoses are: {diagnoses}')
    print('parallel_DMRSD_I1D1R1:: number of steps: ', number_of_steps)
    # rank the diagnoses - can choose the step size for the gradient descent
    print(f'parallel_DMRSD_I1D1R1:: running ranking')
    t2 = time.time()
    ranked_diagnoses, info_sent_ranking = methods_for_ranking.ranking_1(local_spectra, diagnoses, 0.5)
    # sort the diagnoses according to their rank descending
    ranked_diagnoses.sort(key=lambda diag: diag[1], reverse=True)
    t3 = time.time()
    # ranking time is not part of the experiment comparing to the original algorithm
    delta_rank = 0
    # delta_rank = t3 - t2
    # print(f'parallel_DMRSD_I1D1R1:: ranked diagnoses are: {ranked_diagnoses}')
